...
template <typename ResultType> class Caller : public ICaller {
public:
...
template <> class Caller<void> : public ICaller {
public:
  template <typename F, typename... Args>
...
public:
  ...
  auto operator()(F &&f, Args... args)
    ...
    if (threads_.empty())
      throw std::logic_error("No active threads");
    typedef typename std::result_of<F(Args...)>::type ResultType;
    Caller<ResultType> *c =
        new Caller<ResultType>(std::forward<F>(f), std::forward<Args>(args)...);
    std::future<ResultType> ft = c->GetFuture();
    queue_.Push(std::move(std::unique_ptr<ICaller>(c)));
    return ft;
  }
  ...
  void Stop(bool clearQueue = true) { // blocking
    for (int t = 0; t != threads_.size(); ++t)
      queue_.Push(std::move(std::unique_ptr<ICaller>(new Caller<void>)));
...
int main(int argc, char **argv) {
  try {
    ...
    if (f1.get() != 10 || f2.get() != 100 || f3.get() != 1000) {
      std::cerr << "FAILED\n";
      return EXIT_FAILURE;
    }
    std::cout << "OK\n\n";
    ...
    for (int t = 0; t != numtasks; ++t) {
      exec(
          [&iomutex, &counter, &sleeptime_ms](int i) {
            {
              std::lock_guard<std::mutex> lk(iomutex); // also serializes
                                                       // access to
                                                       // counter
              std::cout << "Hello from task: " << i << "\t(" << std::hex
    ...
    std::for_each(counter.begin(), counter.end(), [](const CounterEntry &e) {
      std::cout << "Thread " << std::hex << e.first << std::dec << " executed "
                << e.second << " times\n";
    });
